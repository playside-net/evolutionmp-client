...
use cgmath::Vector2;
pub use native::ui::{
    hide_loading_prompt, set_cursor_sprite
};
...
pub fn show_loading_prompt(ty: LoadingPrompt, text: &str) {
    native::ui::set_loading_prompt_text_entry("STRING");
    native::ui::push_string(text);
    native::ui::show_loading_prompt(ty as u32);
}
    ...
    where P: Into<Vector2<f32>>, S: Into<Vector2<f32>>, C: Into<Rgba>
{
    ...
    native::ui::draw_rect(pos + (size * 0.5), size, color.into())
}
