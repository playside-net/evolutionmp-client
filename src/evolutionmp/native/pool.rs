...
static mut ADDRESS_TO_HANDLE: Option<GetAddressHandle> = None;
...
pub(crate) unsafe fn init(mem: &MemoryRegion) {
    ...
    ADDRESS_TO_HANDLE = Some(std::mem::transmute(mem.find("48 F7 F9 49 8B 48 08 48 63 D0 C1 E0 08 0F B6 1C 11 03 D8")
...
pub type GetAddressHandle = unsafe extern "C" fn(*mut u8) -> Handle;
...
pub trait Pool<T: FromHandle> {
    ...
    fn get_handle(&self, index: u32) -> Option<Handle> {
        if self.is_valid(index) {
            let handle = unsafe { (ADDRESS_TO_HANDLE.unwrap())(self.get_address(index)) };
            Some(handle)
        } else {
            None
        }
    ...
    fn get(&self, index: u32) -> Option<T> {
        self.get_handle(index).and_then(|h| unsafe { T::from_handle(h) })
    }
...
pub trait FromHandle {
    unsafe fn from_handle(handle: Handle) -> Option<Self> where Self: Sized;
}